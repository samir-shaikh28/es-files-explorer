...
import com.droidtechlab.filemanager.ui.activities.superclasses.BasicActivity;
...
import com.droidtechlab.filemanager.utils.EditTextColorStateUtil;
import com.droidtechlab.filemanager.utils.SimpleTextWatcher;
import com.droidtechlab.filemanager.utils.SmbUtil;
...
public class SmbConnectDialog extends DialogFragment {
    ...
    public interface SmbConnectionListener {
         ...
         *     now. See {@link com.droidtechlab.filemanager.database.UtilsHandler#removeSmbPath(String,
         *     String)}
         */
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        ba3.onPositive(
                (dialog, which) -> {
                    ...
                    if (ch.isChecked()) smbFile = createSMBPath(new String[] {ipa, "", "", domaind}, true);
                    else {
    ...
    private SmbFile createSMBPath(String[] auth, boolean anonym) {
        try {
            ...
            String path =
                    "smb://"
                            + (android.text.TextUtils.isEmpty(domain)
                            ? ""
                            : (URLEncoder.encode(domain + ";", "UTF-8")))
                            + (anonym
                            ? ""
                            : (URLEncoder.encode(auth[1], "UTF-8")
                            + ":"
