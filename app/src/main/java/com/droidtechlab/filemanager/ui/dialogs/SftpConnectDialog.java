...
import static com.droidtechlab.filemanager.filesystem.ssh.SshClientUtils.deriveSftpPathFrom;
...
import com.droidtechlab.filemanager.asynchronous.asynctasks.ssh.GetSshHostFingerprintTask;
import com.droidtechlab.filemanager.asynchronous.asynctasks.ssh.PemToKeyPairTask;
...
public class SftpConnectDialog extends DialogFragment {
  ...
  public Dialog onCreateDialog(Bundle savedInstanceState) {
    ...
    portET.setOnFocusChangeListener(
        (v, hasFocus) -> {
    ...
    selectPemBTN.setOnClickListener(
        v -> {
                        ...
                        taskResult -> {
                          ...
                          if (hostKey != null) {
                            ...
                            new AlertDialog.Builder(context)
                                ...
                                .setNegativeButton(
                                    R.string.no, (dialog1, which1) -> dialog1.dismiss())
                                .show();
                          }
                        })
                    .execute();
    ...
    if (!isEdit) {
      try {
        SSHClient result =
            SshConnectionPool.getInstance()
                .getConnection(
                    hostname, port, hostKeyFingerprint, username, password, selectedParsedKeyPair);
  ...
  private String getPemContents() {
    ...
    try {
      BufferedReader reader =
          new BufferedReader(
              new InputStreamReader(context.getContentResolver().openInputStream(selectedPem)));
