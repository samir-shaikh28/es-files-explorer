...
public class PemToKeyPairTask extends AsyncTask<Void, IOException, KeyPair> {
  ...
  public PemToKeyPairTask(@NonNull InputStream pemFile, AsyncTaskResult.Callback<KeyPair> callback)
      throws IOException {
  ...
  protected void onProgressUpdate(IOException... values) {
    ...
    MaterialDialog.Builder builder =
        new MaterialDialog.Builder(AppConfig.getInstance().getMainActivityContext());
    View dialogLayout =
        View.inflate(
            AppConfig.getInstance().getMainActivityContext(), R.layout.dialog_singleedittext, null);
  ...
  private void toastOnParseError(IOException result) {
    Toast.makeText(
            ...
            AppConfig.getInstance()
                .getResources()
                .getString(R.string.ssh_pem_key_parse_error, result.getLocalizedMessage()),
            Toast.LENGTH_LONG)
        .show();
  }
